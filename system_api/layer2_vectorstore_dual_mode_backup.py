...
class Layer2VectorStore:
    ...
    def __init__(
        ...
        Args:
            ...
            use_reranker: Whether to use Cross-Encoder re-ranking instead of FAISS
            reranker_config: Configuration for Cross-Encoder (model, ollama_base_url, etc.)
        """
        ...
        # Re-ranking components (lazy initialization)
        self._reranker = None
        self._document_store = None
        self._reranker_config = reranker_config or {}
        
        if use_reranker:
            logger.info("Layer 2 VectorStore initialized with Cross-Encoder re-ranking mode")
            self._init_reranking_components()
        else:
            logger.info("Layer 2 VectorStore initialized with FAISS mode")
    
    def _init_reranking_components(self):
        """
        Initialize Cross-Encoder re-ranker and document store (lazy)
        """
        try:
            ...
            document_store_path = os.path.join(self.vectorstore_path, "chunks.jsonl")
            cache_size_limit_mb = self._reranker_config.get('cache_limit_mb', 100)
            
            self._document_store = JSONLDocumentStore(
                file_path=document_store_path,
                cache_size_limit_mb=cache_size_limit_mb
            )
            
            logger.info("✓ Cross-Encoder re-ranker initialized")
            logger.info(f"  Model: {model_name}")
            logger.info(f"  Document store: {document_store_path}")
            
        except Exception as e:
            logger.error(f"Failed to initialize re-ranking components: {e}", exc_info=True)
            logger.warning("Falling back to FAISS mode")
    ...
    def build_index(self, documents: List[Document]) -> bool:
        ...
        try:
            ...
            # If using re-ranker, store documents in JSONL and skip FAISS
            if self.use_reranker and self._document_store:
                logger.info(f"Building Layer 2 document store (JSONL) with {len(valid_docs)} chunks...")
                ...
                if success:
                    ...
                    logger.info(f"✓ Layer 2 document store built in {duration:.2f}s")
                    logger.info(f"  Chunks: {self._chunk_count}")
                    logger.info(f"  Papers: {self._paper_count}")
                    logger.info(f"  Mode: Cross-Encoder Re-ranking")
                    
                    return True
                else:
                    logger.error("Failed to build document store")
                    return False
            
            # FAISS mode (original behavior)
    ...
    def search(
        ...
        **kwargs
        ...
        If use_reranker=True: Uses Cross-Encoder re-ranking
        If use_reranker=False: Uses FAISS vector search (original behavior)
        ...
        """
        # Re-ranking mode
        if self.use_reranker and self._reranker and self._document_store:
            return self._search_with_reranking(query, k, filter_paper_ids, **kwargs)
    ...
    def _search_with_reranking(
        ...
        **kwargs
        ...
        """
        Search using Cross-Encoder re-ranking
        
        Args:
            query: User query
            k: Number of results to return
            filter_paper_ids: Optional paper IDs to filter by
            **kwargs: Additional parameters (max_candidates for re-ranking)
            
        Returns:
            List of Document objects, re-ranked by relevance
        """
        try:
            logger.debug(f"Layer 2 re-ranking search: query='{query[:50]}...', k={k}")
            
            # Get candidate chunks from document store
            if filter_paper_ids:
                candidates = self._document_store.get_chunks_by_paper_ids(filter_paper_ids)
                logger.debug(f"  Retrieved {len(candidates)} candidates from {len(filter_paper_ids)} papers")
            else:
                # Get all chunks (might be expensive)
                candidates = self._document_store.get_chunks_by_paper_ids(None)
                logger.debug(f"  Retrieved all {len(candidates)} chunks")
            
            if not candidates:
                logger.warning("No candidates found for re-ranking")
                return []
            
            # Limit candidates if too many
            max_candidates = kwargs.get('max_candidates', 300)
            if len(candidates) > max_candidates:
                logger.debug(f"  Limiting to {max_candidates} candidates (from {len(candidates)})")
                # For now, take first N (could use BM25 pre-filtering here)
                candidates = candidates[:max_candidates]
            
            # Convert to dict format for reranker
            # candidates are already dicts from document store
            chunk_dicts = [
                {
                    'text': chunk.get('text', ''),
            ...
            ranked_results = self._reranker.rank_chunks(query, chunk_dicts, top_k=k)
            
            # Convert back to Document objects
            results = []
            for chunk_dict, score in ranked_results:
                doc = Document(
                    page_content=chunk_dict['text'],
                    metadata=chunk_dict['metadata']
                )
                # Store score in metadata for potential later use
                doc.metadata['rerank_score'] = score
                results.append(doc)
            
            logger.debug(f"✓ Layer 2 re-ranking returned {len(results)} chunks")
            if results:
                logger.debug(f"  Top score: {results[0].metadata.get('rerank_score', 0):.4f}")
    ...
    def search_with_scores(
        ...
        **kwargs
        ...
        # Re-ranking mode
        if self.use_reranker and self._reranker and self._document_store:
            # Use re-ranking search
            docs = self._search_with_reranking(query, k, filter_paper_ids, **kwargs)
    ...
    def add_chunks(self, chunks: List[Document]) -> bool:
        ...
        Supports both FAISS and re-ranking modes.
        ...
        try:
            ...
            for chunk in chunks:
                ...
                if not chunk.page_content or len(chunk.page_content) < 50:
                    logger.warning("Chunk content too short, skipping")
                    continue
            ...
            # Re-ranking mode: append to document store
            if self.use_reranker and self._document_store:
                logger.info(f"Adding {len(valid_chunks)} chunks to document store...")
                
                # Read existing chunks
                existing_chunks = self._document_store.get_chunks_by_paper_ids(None)
                
                # Convert new chunks to dict format
                new_chunk_dicts = []
                ...
                # Combine and store all chunks
                all_chunks_data = existing_chunks + new_chunk_dicts
                
                # Re-create documents for storage
                all_docs = []
                ...
                # Store all chunks (overwrites file)
                success = self._document_store.store_chunks(all_docs)
                
                if success:
                    self._chunk_count += len(valid_chunks)
                    logger.info(f"✓ Added {len(valid_chunks)} chunks to document store")
                    return True
                else:
                    logger.error("Failed to add chunks to document store")
                    return False
            
            # FAISS mode: add to vectorstore
            if not self.vectorstore:
                logger.error("VectorStore not initialized. Call build_index() or load() first.")
                return False
            ...
            logger.info(f"Added {len(valid_chunks)} chunks to Layer 2 (FAISS)")
            
            return True
            
        except Exception as e:
            logger.error(f"Failed to add chunks to Layer 2: {e}", exc_info=True)
            return False
